...
############## Clustering ##############################
########################################################
cor(scca_brain[, 1], scca_brain[,2])
scca_brain <- scale(feature_brain_reduced) %*% res.wei.syn$u
cca_brain_1 <- scca_brain[, 1:2]
...
############ Fuzzy C means clustre analysislibrary(cluster)
...
RS_stable <- data.frame(CV1 = unlist(cca_brain_1[idx_clu, 1]), CV2 = unlist(cca_brain_1[idx_clu, 2]))
RS_stable$clusters <- as.factor(clu_stable$Hclu)

# 3D figures
plot_ly(RS_stable, x = ~CV1, y = ~CV2, z = ~ CV3,
        color = ~clusters, marker = list(size=3.5), colors = c("skyblue2", "gold1"))
# 2D figure
ggplot(RS_stable) + geom_point(aes(x = CV1, y = CV2, color = clusters)) +
  scale_color_manual(values = c("skyblue2", "gold1"))+
  labs(x ="Canonical variate 1", y = "Canonical variate 2") + 
  theme_bw() 
